...
!developmentChains.includes(network.name)
    ...
    : describe("Raffle Unit Tests", async function () {
          ...
          describe("Enter Raffle", async function () {
              ...
              it("Records player when they enter the raffle", async function () {
          ...
          describe("performUpkeep function", async function () {
              it("will run only if checkupkeep is true", async function () {
                  await raffle.enterRaffle({ value: raffleEntranceFee })
                  await network.provider.send("evm_increaseTime", [interval.toNumber() + 1])
                  await network.provider.send("evm_mine", [])
              ...
              it("will not run if checkupkeep is false", async function () {
          ...
          describe("fulfillRandomWords function", async function () {
              beforeEach(async function () {
                  await raffle.enterRaffle({ value: raffleEntranceFee })
                  await network.provider.send("evm_increaseTime", [interval.toNumber() + 1])
                  await network.provider.send("evm_mine", [])
              ...
              it("picks a winner, resets the lottery, and sends money", async function () {
                  ...
                  await new Promise(async (resolve, reject) => {
                      raffle.once("WinnerPicked", async () => {
                          ...
                          try {
                              const recentWinnter = await raffle.getRecentWinner()
                              const winnerEndingBalance = await accounts[1].getBalance()
                              console.log("Recent winner: ", recentWinnter)
